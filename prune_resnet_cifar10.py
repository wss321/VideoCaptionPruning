...
parser.add_argument('--model', type=str, required=True, choices=['resnet34', 'reesnet18', 'resnet50'])
...
handler = logging.FileHandler(f"output/log-{args.mode}-{args.model}.txt", 'a')
    ...
    for epoch in range(args.start_epoch, args.total_epochs):
        ...
        for i, (img, target) in enumerate(train_loader):
            ...
            if i % 10 == 0 and args.verbose:
                logger.info("Epoch %d/%d, iter %d/%d, loss=%.4f, lr=%.7f" % (
        ...
        if best_acc < acc:
            ...
            torch.save(model, f'checkpoints/{model.__class__.__name__}/{args.model}-{args.mode}-round{args.round}.pth')
            best_acc = acc
        scheduler.step()
...
def main():
    ...
    if args.mode == 'train':
        ...
        ckpt = f'checkpoints/ResNet/{args.model}-round{args.round}.pth'
        if args.resume:
        ...
        previous_ckpt = f'checkpoints/ResNet/{args.model}-round{args.round}.pth'
